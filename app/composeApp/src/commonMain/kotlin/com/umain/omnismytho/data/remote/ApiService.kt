...
class ApiService(private val client: HttpClient) {
    ...
    suspend fun getMythologies(): List<MythologyDto> =
        client.get("$BASE_URL/mythologies").body()

    suspend fun getMythology(id: String): MythologyDetailDto =
        client.get("$BASE_URL/mythologies/$id").body()
    ...
    suspend fun searchEntities(query: String, limit: Int = 10): List<EntityDto> =
        client.get("$BASE_URL/entities/search") {
            parameter("q", query)
            parameter("limit", limit)
        }.body()

    suspend fun getEntity(id: String): EntityDto =
        client.get("$BASE_URL/entities/$id").body()
}
